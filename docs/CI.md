...
**フロントはホストで `npm` しない。** `node_modules` は bind mount 共有のため、Mac で `npm install` したあとコンテナで `npm ci` すると `ENOTEMPTY` になりやすいです。依存は常に `node` コンテナ内で入れます。
...
一括実行: `./scripts/check-quality.sh`（内部で `rm -rf node_modules && npm ci` してから lint / build）
...
フロント（ESLint / Vite build / Newman）は **上記 Docker コマンドを使う**。GitHub Actions は毎回クリーンな VM 上で `npm ci` するため、手元の `ENOTEMPTY` は主に「ホストと Docker で `node_modules` を混在させたとき」に発生します。
...
| `npm:docker-ci` | `rm -rf node_modules && npm ci`（`node` コンテナ） | （ローカル手動） |
| `npm:docker-build` | 上記 + `npm run build` | （ローカル / `setup`） |
...
### `npm error ENOTEMPTY`（`node_modules/rxjs` など）

**原因:** ホストで作った `node_modules` をマウントしたまま、コンテナ内で `npm ci` が削除しきれない。
...
rm -rf node_modules
composer npm:docker-build
# または ./scripts/check-quality.sh（先頭で rm -rf && npm ci）
```

以降はホストで `npm install` / `npm ci` しない。
