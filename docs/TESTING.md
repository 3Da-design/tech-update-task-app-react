...
Laravel のテストランナーで、認証・バリデーション・CRUD など **アプリケーションの仕様** を自動検証します。テスト DB は **PostgreSQL**（Docker Compose / CI とも開発環境と同一エンジン）。

### 構成

| ファイル | 説明 |
|---------|------|
| `phpunit.xml` | テストスイート（DB 接続は `.env` / 環境変数の PostgreSQL を使用） |
| `tests/Feature/TaskApiTest.php` | Task API の主テスト |
| `tests/Feature/TaskWebTest.php` | Task Web（Blade）の主テスト |
| `tests/Feature/TaskListFilterTest.php` | 一覧の検索・フィルタ・期限ソート |
...
| `test_guest_cannot_access_api_tasks` | 未認証 → 401 |
...
### Task 一覧フィルタ（`TaskListFilterTest`）

| テスト | 期待 |
|--------|------|
| `test_*_filters_by_title_partial_match` | タイトル部分一致（PostgreSQL では検索文字列の大文字小文字を一致させる） |
| `test_*_filters_by_status` | ステータス絞り込み |
| `test_*_sorts_due_date_asc_with_nulls_first` | 期限昇順・NULL 先頭 |
| `test_*_sorts_due_date_desc` | 期限降順 |

大文字小文字を区別しない検索のテストは [db-schema-change シナリオ](./experiment/scenarios/db-schema-change.md) 開始時に追加する。
